...
async function handleEvent(events: any[], isAsync: boolean, resp: Response) {
  ...
  if (!isAsync) {
    if (events.length > 1) {
      let errMsg =
        "Invalid request format: multiple events provided for synchronous request";
      error = {
        err_type: RuntimeErrorType.InvalidRequest,
        err_msg: errMsg,
      } as RuntimeError;
  ...
  for (let event of events) {
    let result;
    const functionName: FunctionFactoryType = event.execution_metadata
      .function_name as FunctionFactoryType;
...
function handleActivateHookResult(
  event: any,
...
function handleDeactivateHookResult(
  ...
  if (
    (handlerError !== undefined || res?.status === "error") &&
    !event.payload.force_deactivate
  ) {
    console.debug("Setting snap-in status to error");
    update_req.status = SnapInsSystemUpdateRequestStatus.Error;
  } else {
...
async function updateSnapInState(
  event: any,
  update_req: SnapInsSystemUpdateRequest,
) {
...
function getActivateHookResult(input: any): ActivateHookResult {
  ...
  if (input instanceof Object) {
    if (input.status === "active" || input.status === "error") {
      ...
      console.error(
        `Invalid status field ${input.status}: status must be active or error`,
      );
    }
    if (input.inputs_values instanceof Object) {
      ...
      console.error(
        `Invalid inputs_values field ${input.inputs_values}: inputs_values is not an object`,
      );
    }
  }
  return res;
...
function getDeactivateHookResult(input: any): DeactivateHookResult {
  ...
  if (input instanceof Object) {
    if (input.status === "inactive" || input.status === "error") {
      ...
      console.error(
        `Invalid status field ${input.status}: status must be inactive or error`,
      );
    }
    if (input.inputs_values instanceof Object) {
      res.inputs_values = input.inputs_values;
